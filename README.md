...
2. 运行代码：python Bilicomment.py（或pycharm等软件打开运行）。代码使用selenium + chrome浏览器爬取数据。
...
4. 在使用selenium + chrome浏览器爬取数据时，如果并发任务过多，或者爬虫的运行时间很长，那么很容易出现浏览器崩溃的现象。这一般是因为资源消耗过大造成的，selenium模拟浏览器会产生大量的临时文件，浏览器缓存过大，越堆越多。目前程序遇到浏览器崩溃会自动重启浏览器。为了在重启浏览器之前删除缓存和释放空间，程序可以将缓存存储在一个指定的临时目录中，然后在重启浏览器前删除该目录。
