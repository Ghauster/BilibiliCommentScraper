...
## 功能
1. 爬取一级和二级评论:包含以下评论信息：编号、隶属关系、被评论者昵称、被评论者 ID、昵称、用户 ID、评论内容、发布时间、点赞数。
2. 自动滚动页面以加载更多评论,处理翻页操作以获取更多二级评论。
3. 断点续爬：中断后，爬虫可以根据 progress.txt 文件中的进度继续爬取。如果想要从头开始爬取，只需删除 progress.txt 文件即可。
