...
3. 爬取超大评论量的热门视频时，网页可能会因为内存不足(网页显示"喔唷,崩溃啦""out of memory")而崩溃。如果发生这种情况，程序会在一定时间后**自动重启浏览器断点续爬**。但是如果网页都还没有滚动到底全部加载完、都还没有开始爬，就内存不足了，那无论自动重试多少次都会重复出现网页崩溃的问题，此时建议限制最大滚动次数。
